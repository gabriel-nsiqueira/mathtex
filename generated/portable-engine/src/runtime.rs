...
// Style context for a host box request, display math collapses to Text to match cursize.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
// Glyph offsets are scaled points from the box baseline origin, y positive downward.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
// Rule origin is its bottom left corner with y positive downward, height extends upward.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
// \Uhostbox is extension cmd 59 with this chr, upstream extension codes stop at 46.
pub(crate) const HOST_BOX_EXTENSION_CODE: i32 = 91;
// Marker whatsit subtypes: pending carries the opaque token id, resolved carries a record index.
pub(crate) const HOST_BOX_PENDING_SUBTYPE: i16 = 91;
pub(crate) const HOST_BOX_RESOLVED_SUBTYPE: i16 = 92;
// eqtb slot of the current font in this import's layout (see the mainf reads in maincontrol).
pub(crate) const EQTB_CUR_FONT_LOC: i64 = 1206823;
// eqtb base of the family 2 math symbol fonts, indexed by size 0/256/512 (see zmathquad).
...
pub struct PortableTexEngine<'resources> {
    ...
    // Host box render payloads by record index, markers in node memory carry only the index.
...
impl<'resources> PortableTexEngine<'resources> {
    ...
    pub fn begin_fragment_capture(self: &mut Self) {
        ...
        // Record indices in marker nodes are per fragment, a reused engine must not accumulate.
    ...
    /// Host box render payload by record index, as referenced by `HostBoxRef` marker nodes.
    ...
    pub fn snapshot_node(&self, handle: PortableNodeHandle) -> Option<PortableNodeSnapshot> {
        ...
        let character = if is_character {
            ...
            // Host box markers carry their record index in the payload word.
            self.node_word(node, 1).map_or(0, |word| unsafe { word.u.CINT })
        } else {
    ...
    // Registers \Uhostbox during format initialization so it dumps with the format eqtb and hash.
    pub(crate) unsafe fn register_host_box_primitive(self: &mut Self) -> EngineFlow<()> {
        ...
        // eq_level one, eq_type extension (59), equiv = the host box extension chr code.
    ...
    // Allocates a size 2 marker whatsit whose payload is a pending token id or a resolved record index.
    unsafe fn host_box_marker(
        ...
        let this = &mut *engine;
        let marker = this.zgetnode(2 as i32)?;
        ...
        (*mem.offset((marker as i32 + 1 as i32) as isize)).u.CINT = payload;
        Ok(marker)
    }

    // Asks the host for the box behind `token`, None falls back to a deterministic zero size box.
    unsafe fn host_box_build(
        ...
        let marker =
            Self::host_box_marker(engine, HOST_BOX_RESOLVED_SUBTYPE, index as integer)?;
        (*mem.offset((hbox as i32 + 5 as i32) as isize)).hh.v.RH = marker;
        Ok(hbox)
    }

    // \Uhostbox scan site: math defers to the mlist pass for style, other modes resolve now as Text.
    pub(crate) unsafe fn host_box_insert(
        ...
        if mode == 209 as i32 {
            let marker =
                Self::host_box_marker(engine, HOST_BOX_PENDING_SUBTYPE, token)?;
            let this = &mut *engine;
    ...
    // mlist pass hook: swap a pending placeholder nucleus for the host's box under curstyle.
    pub(crate) unsafe fn host_box_resolve_noad(
        ...
        let mem: *mut memoryword = this.state.zmem.as_mut_ptr();
        // Only Ord noads (16) carry a \Uhostbox placeholder nucleus.
    ...
    // clean_box hook: fields copied out of single atom groups (scripts, fractions) resolve here.
    pub(crate) unsafe fn host_box_resolve_field(
        ...
        let mem: *mut memoryword = this.state.zmem.as_mut_ptr();
        // Field must be a sub box (2) whose box wraps a pending marker whatsit.
        ...
        let token = (*mem.offset((marker as i32 + 1 as i32) as isize)).u.CINT;
        // Style codes map to sizes as in mlist_to_hlist: below 4 is text, then script sizes.
        ...
        // Size context comes from the family 2 symbol font at the active math size.
        let eqtb = this.state.zeqtb.as_mut_ptr();
        ...
        // The placeholder was stamped with the \hostbox call site span at scan time, carry it onto
        // the replacement and its marker so the atom maps to its own bytes, not the ambient span.
