...
impl<'resources> PortableTexEngine<'resources> {
    ...
    pub fn snapshot_node(&self, handle: PortableNodeHandle) -> Option<PortableNodeSnapshot> {
        ...
        let character = if is_character {
            ...
            // Host box markers carry their record index in the payload word after the metrics.
    ...
    // Allocates a size 5 marker whatsit in the glyph node metric layout (width/depth/height at
    // words 1..3) so hpack measures it even after rebox dissolves the enclosing shell box.
    // Word 4 holds the payload, a pending token id or a resolved record index.
